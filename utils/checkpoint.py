    ...
    if inflation:
        ...
        for k, v in ckpt_state.items():
            if k in model_state and model_state[k].shape == v.shape:
                ...
                skipped_shape.append("{} {} vs {}".format(k, tuple(v.shape), tuple(model_state[k].shape)))
        if skipped_shape:
            logger.info("Skipped {} keys due to shape mismatch (e.g. {} ...)".format(
                len(skipped_shape), skipped_shape[0] if skipped_shape else ""))
        mismatch = ms.load_state_dict(filtered_ckpt, strict=False)
        logger.info("Keys in model not matched: {}".format(mismatch[0]))
        logger.info("Keys in checkpoint not matched: {}".format(mismatch[1]))
        if "model_ema_state" in checkpoint.keys() and model_ema is not None:
            logger.info("Loading model ema weights.")
            ema_state = checkpoint["model_ema_state"]
            ema_model_state = ms_ema.state_dict()
            filtered_ema = {k: v for k, v in ema_state.items()
                           if k in ema_model_state and ema_model_state[k].shape == v.shape}
            ms_ema.load_state_dict(filtered_ema, strict=False)
            logger.info("Keys in model not matched: {}".format(mismatch[0]))
            logger.info("Keys in checkpoint not matched: {}".format(mismatch[1]))
        else:
        ...
        if optimizer and "optimizer_state" in checkpoint.keys():
            try:
                ...
                logger.warning(
                    "Optimizer state not loaded (model architecture may differ from checkpoint): {}".format(e)
                )
