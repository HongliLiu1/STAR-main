...
class Split_few_shot():
    ...
    def __init__(self, folder, split_dataset='train', dataset="Ssv2_few_shot"):
        ...
        if dataset == 'Ssv2_few_shot':
            ...
            for class_folder in folder:
                # train0//videos/ApplyEyeMakeup/v_ApplyEyeMakeup_g01_c01.avi
                #  self.split_dataset = 'train'
                # class_folder.strip().split('//') = ["train0", "videos/ApplyEyeMakeup/v_ApplyEyeMakeup_g01_c01.avi"]
                # [-1] = "videos/ApplyEyeMakeup/v_ApplyEyeMakeup_g01_c01.avi"
                paths = class_folder.strip().split('//')[-1]
                # class_folder.strip().split('//')[0] = "train0"
                # class_id = "0"
                class_id = int(class_folder.strip().split('//')[0][len(split_dataset):]) # class_folders.index(class_folder)
                self.add_vid(paths, class_id)
...
class Ssv2_few_shot(BaseVideoDataset):
    ...
    def _get_sample_info(self, index):
        ...
        """
       # print(self._samples[index])
        if self.dataset_name!="Ssv2_few_shot":
            class_ = self._samples[index].strip().split('//')[0][len(self.split_dataset):]
            video_path = os.path.join(self.data_root_dir, self._samples[index].strip().split('//')[-1] )
        else:
            class_ = int(self._samples[index].strip().split('/')[0][len(self.split_dataset):])
        # class_ = self._samples[index]["label_idx"]
        #class_folder.strip().split('//')[-1]
        #video_path = os.path.join(self.data_root_dir, self._samples[index]["id"]+".mp4")
            video_path = os.path.join(self.data_root_dir, self._samples[index].strip().split('/')[-1] + ".mp4")
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.TRAIN.META_BATCH:
            ...
            # get_unique_classes(self) --list(set(self.gt_a_list))
            required_samples = self.cfg.TRAIN.SHOT + self.cfg.TRAIN.QUERY_PER_CLASS_TEST
            valid_classes = [
                cls_id for cls_id in classes
                if c.get_num_videos_for_class(cls_id) >= required_samples
            ]
            # print("Valid classes and names:")
            # for cls_id in valid_classes:
                # print(f"Class ID: {cls_id}, Name: {self.cfg.TEST.CLASS_NAME[cls_id]}")
            # selected_class_indices = [16,18,9,15,5]
            #valid_classes
            batch_classes = random.sample(valid_classes , self.cfg.TRAIN.WAY)
            ...
            for retry in range(retries):
                try:
                    ...
                    for bl, bc in enumerate(batch_classes):
                        ...
                        # print(f"class {bc}: total={n_total}, required={SHOT + n_queries}")
                        idxs = random.sample([i for i in range(n_total)], SHOT + n_queries)
                        #print(idxs)
                        for idx in idxs[0:SHOT]:
                            #AUGMENTATION:COLOR_AUG: false NO_RANDOM_ERASE: true
                            if (hasattr(self.cfg.AUGMENTATION, "SUPPORT_QUERY_DIFF_SUPPORT") and
                                    self.cfg.AUGMENTATION.SUPPORT_QUERY_DIFF_SUPPORT and
                                    self.split_dataset=="train"):
           ...
           # print( f"support_set: {support_set}, support_labels: {support_labels}, real_support_labels: {real_support_labels}")
            s = list(zip(support_set, support_labels, real_support_labels))
            random.shuffle(s)
            support_set, support_labels, real_support_labels = zip(*s)
            t = list(zip(target_set, target_labels, real_target_labels, target_vid_ids))
            random.shuffle(t)
            target_set, target_labels, real_target_labels, target_vid_ids = zip(*t)
            ...
            return {"support_set":support_set, "support_labels":support_labels, "target_set":target_set, "target_labels":target_labels, "real_target_labels":real_target_labels,
                    "batch_class_list": batch_classes, "real_support_labels":real_support_labels,"target_vid_ids": target_vid_ids}
    ...
    def get_seq(self, label, idx=-1):
        ...
        if self.cfg.TRAIN.META_BATCH:
            ...
            if self.dataset_name == 'Ssv2_few_shot':
                ...
                # "/mnt/disk1/liuhongli/downdatas/UCF101/videos/PlayingDhol/v_PlayingDhol_g11_c05.avi"
