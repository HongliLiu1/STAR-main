...
class BaseVideoDataset(torch.utils.data.Dataset):
    ...
    def _interval_based_sampling(self, vid_length, vid_fps, clip_idx, num_clips, num_frames, interval):
        # 🔒 Step 1: fallback for short videos
        if vid_length < num_frames:
            index = torch.linspace(0, max(vid_length - 1, 0), num_frames)
            index = torch.clamp(index, 0, vid_length - 1).long()
            return index

        # 🔁 Step 2: special case - single frame
        if num_frames == 1:
            return [random.randint(0, vid_length - 1)]

        # 🧠 Step 3: handle train vs test with optional interval override
        if self.split == "train" and hasattr(self.cfg.DATA, "SAMPLING_RATE_TRAIN"):
            interval = max(1, vid_length // num_frames)
            clip_length = interval * num_frames
        ...
        elif self.cfg.DATA.SAMPLING_RATE > 40:
            interval = max(1, vid_length // num_frames)
            clip_length = interval * num_frames
